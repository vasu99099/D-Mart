...
const SideCartProductCard = ({ product }) => {
  console.log(product);
  const dispatch = useDispatch();
  const { MRP, D_mart_Price, stock } = product;
  ...
  /**
   * The function add the  product into cart
   * It will call when product is not in cart
   */
  ...
  return (
    <div className=" p-1 row col-12 mx-0 mb-3  ">
      <div
        className={
          shoppingCartQty && shoppingCartQty > 0
            ? "card border-success"
            : "card"
        }
      >
        <div className="d-flex">
          ...
          <div className="card-body px-0 px-lg-2 ms-3">
            ...
            <div className="d-flex justify-content-between">
              <div>
                <div className="">
                  <p className="my-0 mt-3">
                    {" "}
                    you pay : <strong>₹ {D_mart_Price}</strong>
                  </p>
                </div>
                <div>
                  {" "}
              ...
              <div className="text-success bg-light fw-bold rounded px-2 mt-5">
                <p className="my-0 "> You Save ₹ {MRP - D_mart_Price}</p>
              </div>
            </div>
          </div>
        </div>
...
export default SideCartProductCard;
